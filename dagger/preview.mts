...
async function githubCreateComment(body: string) {
  ...
  const { data } = await authedOctokit.request('/user');
}
...
function app(c: Client): Container {
  ...
  const app = node
    ...
    .withDirectory(
      '/app',
      c.host().directory(src, { exclude: ['node_modules, .next'] })
    )
...
export function createClient() {
  return connect(
    async (client: Client) => {
      const toto = await githubCreateComment('Creating Dagger client');
